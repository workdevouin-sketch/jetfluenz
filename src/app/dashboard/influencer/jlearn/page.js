...
export default function JLearnPage({ role = 'influencer' }) {
    const [dailyTip, setDailyTip] = useState(null);
    const [randomTip, setRandomTip] = useState(null);
    const [liked, setLiked] = useState(false);

    useEffect(() => {
        setDailyTip(getDailyTip('influencer'));
        setRandomTip(getRandomTip('influencer'));
    ...
    const handleNewRandomTip = () => {
        const newTip = getRandomTip('influencer');
        setRandomTip(newTip);
        setLiked(false);
    };

    if (!dailyTip) return (
        <DashboardLayout role={role} title="JLearn">
            ...
            <div className="max-w-4xl mx-auto space-y-8 pb-12">
                ...
                <div className="text-center space-y-2 mb-10">
                    <h1 className="text-4xl font-extrabold text-[#343C6A] tracking-tight">
                        Social Media <span className="text-[#2008b9]">Mastery</span>
                    </h1>
                    <p className="text-gray-500 text-lg max-w-2xl mx-auto">
                        Daily tips to supercharge your influencer journey.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-[#1a1a1a] to-[#2d2d2d] rounded-[2rem] p-8 text-white relative overflow-hidden flex flex-col justify-between">
                        <div>
                            ...
                            <p className="text-gray-400 text-sm mb-6">Consistency beats perfection every time.</p>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm border border-white/5">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs text-gray-400 font-bold uppercase">Avg. Engagement</p>
